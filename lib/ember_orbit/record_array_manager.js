  ...
  _linkWasAdded: function(record, key, value) {
    ...
    if (linkType) {
      ...
      if (links && relatedRecord) {
        var recordArrays = this.recordArraysForRecord(relatedRecord);
        recordArrays.add(links);
  ...
  */
  createManyArray: function(record, key, relatedRecords) {
    ...
    var manyArray = HasManyArray.create({
      ...
      _parentId: record[get(this.context, 'idField')],
      _parentType: record.constructor.typeKey,
      _linkKey: key
    });
    ...
    forEach(relatedRecords, function(relatedRecord) {
      var arrays = this.recordArraysForRecord(relatedRecord);

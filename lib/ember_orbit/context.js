...
var Context = Source.extend({
  ...
  didTransform: function(operation, inverse) {
    ...
    if (op === 'replace') {
      console.log('replace', path[0], path[1], path[2], value);
      var record = _this._lookupRecord(path[0], path[1]);
      var prop = path[2];
      record.propertyDidChange(prop);
    }

    // TODO - other ops
  },

  // TODO: return a live recordarray
  all: function(type) {
    var ids = Object.keys(this._source.retrieve([type]));

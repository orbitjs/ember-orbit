...
var Context = Source.extend({
  ...
  getLink: function(record, key) {
    ...
    if (relatedId) {
      ...
      type = get(this, 'schema').linkProperties(record.constructor.typeKey, key).model;

      if (type) {
        promise = this.find(type, relatedId);
        label = "OE: Context#find type " + type + " with id " + relatedId;
      } else {
        promise = this.find(relatedId);
        label = "OE: Context#find single polymorphic record";
      }

      return promiseObject(promise, label);
